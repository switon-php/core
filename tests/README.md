# Switon Core Package Tests

Test suite for the Switon Core package, organized by test type and speed.
...
└── Integration/   # Slower integration tests (file system, etc.)
```
...
# Shorthand: vendor/bin/phpunit --configuration tests/phpunit.xml.dist --testsuite=unit and vendor/bin/phpunit --configuration tests/phpunit.xml.dist --coverage-html tests/coverage
```
...
- **Proper Assertions**: Use specific assertions (`assertDirectoryExists`, etc.)
...
- Prefer `{ThingUnderTest}Test.php` (e.g. `PathAliasTest.php`, `AppTest.php`)
